...
public interface IEditContextValidator
{
    ...
    void ValidateField(EditContext editContext, ValidationMessageStore messages, in FieldIdentifier fieldIdentifier,
        Func<string, IEnumerable<string>, string> messageLocalizer);
}
...
public class EditContextValidator : IEditContextValidator
{
    ...
    /// </summary>
    private readonly ConcurrentDictionary<(Type ModelType, string FieldName), ValidationPropertyInfo>
    ...
    public virtual void Validate(EditContext editContext, ValidationMessageStore messages,
        Func<string, IEnumerable<string>, string> messageLocalizer)
    {
    ...
    /// <inheritdoc/>
    public virtual void ValidateField(EditContext editContext, ValidationMessageStore messages,
        in FieldIdentifier fieldIdentifier, Func<string, IEnumerable<string>, string> messageLocalizer)
    {
        if (TryGetValidatableProperty(fieldIdentifier, out var validationPropertyInfo, messageLocalizer is not null))
        {
            ...
            if (messageLocalizer is not null)
            {
                ...
                Validator.TryValidateValue(propertyValue, validationContext, results,
                    validationPropertyInfo.ValidationAttributes);
                ...
                Validator.TryValidateValue(propertyValue, validationContext, formattedResults,
                    validationPropertyInfo.FormattedValidationAttributes);
                ...
                for (int i = 0; i < results.Count; ++i)
                {
                    ...
                    var errorMessageString =
                        ValidationAttributeHelper.RevertErrorMessagePlaceholders(formattedResults[i].ErrorMessage);
            ...
            else
            {
                ...
                if (editContext.Model is IValidatableObject validatableObject)
                {
                    ...
                    if (validateResult is not null &&
                        validateResult.Any(x => x.MemberNames.Contains(validationContext.MemberName)))
                    {
                        messages.Add(fieldIdentifier,
                            validateResult.Where(x => x.MemberNames.Contains(validationContext.MemberName))
    ...
    protected virtual bool TryGetValidatableProperty(in FieldIdentifier fieldIdentifier,
        out ValidationPropertyInfo validationPropertyInfo, bool forLocalization)
    {
